async function callDB(db, sql) {
  ...
  this.logger.debug(`${this._traceStack()} - Get data from data store`);
...
async function getBatteryData() {
  try {
    ...
    // Arlo battery info
    this.logger.debug(`${this._traceStack()} - Arlo battery info`);
    const sql =
    ...
    this.logger.debug(`${this._traceStack()} - Flower Care battery info`);
    tempResults = await callDB.call(this, 'flowercare', sql);
    if (tempResults.rows[0].battery !== null) results.push(tempResults.rows);

    // Netatmo battery info
    this.logger.debug(`${this._traceStack()} - Netatmo battery info`);
    tempResults = await callDB.call(this, 'netatmo', sql);
    if (tempResults.rows[0].battery !== null) results.push(tempResults.rows);

    // Link-tap
    this.logger.debug(`${this._traceStack()} - Link-tap battery info`);
    const url = 'https://www.link-tap.com/api/getAllDevices';
    const LinkTapUser = await this._getVaultSecret(
      process.env.ENVIRONMENT,
      'LinkTapUser',
    );
    const LinkTapKey = await this._getVaultSecret(
      process.env.ENVIRONMENT,
      'LinkTapKey',
    );
