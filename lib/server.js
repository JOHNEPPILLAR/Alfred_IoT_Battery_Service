...
require('../api/schedules/schedules.js').applyRoutes(server, '/schedule');
...
global.deviceDataClient.on('error', (err) => {
  serviceHelper.log('error', 'server', 'Device data store: Unexpected error on idle client');
  serviceHelper.log('error', 'server', err.message);
  cleanExit();
});

// Setup scheduler
setTimeout(() => { scheduleController.collectData(); }, 1000);
